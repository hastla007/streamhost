...
from app.utils import ObservedLock, preview_assets_lock
...
class LiveStreamManager:
    ...
    def __init__(self) -> None:
        self._lock = ObservedLock(
            "stream_engine_lock", acquire_timeout=settings.lock_acquire_timeout_seconds
        )
    ...
    async def _stop_locked(self) -> None:
        ...
        if self._preview_lock_held:
            try:
                preview_assets_lock.release()
            except RuntimeError:
                logger.warning("Preview asset lock release requested but lock was not held")
            finally:
                self._preview_lock_held = False
    ...
    async def _launch_process(self) -> None:
        ...
        if not self._preview_lock_held:
            try:
                await preview_assets_lock.acquire(settings.lock_acquire_timeout_seconds)
            except asyncio.TimeoutError as exc:
                raise StreamingError("Timed out waiting for preview asset lock") from exc
            self._preview_lock_held = True
        ...
        redacted_command = [
            "<redacted>" if arg == self._plan.destination and arg.startswith("rtmp") else arg
            for arg in command
        ]
        logger.info("Starting FFmpeg", extra={"command": redacted_command})
