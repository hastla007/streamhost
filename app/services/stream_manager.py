...
from app.services.video_processor import hardware_encoder_available
...
class StreamManager:
    ...
    def __init__(self) -> None:
        self._lock = ObservedLock(
            "stream_manager_lock", acquire_timeout=settings.lock_acquire_timeout_seconds
        )
    ...
    def _resolve_encoder(self) -> tuple[str, str]:
        ...
        if hardware in {"nvenc", "qsv", "videotoolbox"}:
            encoder_name = self._encoder_display(hardware)
            if not hardware_encoder_available(encoder_name):
                raise HTTPException(
                    status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
                    detail=f"Requested hardware encoder {encoder_name} is not available on this host",
                )
