...
from app.utils import preview_assets_lock
...
class CleanupService:
    ...
    async def cleanup_all(self) -> dict[str, int]:
        ...
        thumbnail_result, preview_result, concat_result = await asyncio.gather(
            ...
            self._cleanup_preview_segments_guarded(),
    ...
    async def _cleanup_preview_segments_guarded(self) -> tuple[int, int]:
        async with preview_assets_lock:
            return await asyncio.to_thread(self._cleanup_preview_segments)
