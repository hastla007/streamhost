...
async def _save_upload_securely(upload: UploadFile) -> tuple[Path, int, str, str]:
    ...
    declared_size_header = upload.headers.get("content-length") if upload.headers else None
    try:
        expected_size = int(declared_size_header) if declared_size_header else None
    ...
    try:
        ...
        if expected_size is not None and received != expected_size:
            ...
            raise HTTPException(
                ...
                detail="Uploaded file size did not match declared Content-Length",
            )
