...
class Settings(BaseModel):
    ...
    lock_acquire_timeout_seconds: float = Field(
        15.0, validation_alias="LOCK_ACQUIRE_TIMEOUT_SECONDS", ge=0
    )
    ...
    database_pool_reject_threshold: float = Field(
        0.95, validation_alias="DATABASE_POOL_REJECT_THRESHOLD", ge=0.0, le=1.0
    )
    ...
    def validate_admin_password(cls, value: str) -> str:
        ...
        if not (any(c.islower() for c in value) and any(c.isupper() for c in value)):
            raise ValueError("ADMIN_DEFAULT_PASSWORD must include mixed case characters")
        if not any(c.isdigit() for c in value):
            raise ValueError("ADMIN_DEFAULT_PASSWORD must include at least one digit")
        if not any(c in "!@#$%^&*()-_=+[]{};:,./?" for c in value):
            raise ValueError("ADMIN_DEFAULT_PASSWORD must include at least one symbol")
        return value
    ...
    @model_validator(mode="after")
    def ensure_stream_profile_bounds(self) -> "Settings":
        """Validate the relationship between resolution, bitrate and frame rate."""

        width_str, height_str = self.stream_resolution.split("x")
        width = int(width_str)
        height = int(height_str)

        if width >= 7680 or height >= 4320:
            if self.stream_bitrate < 80_000:
                raise ValueError("STREAM_BITRATE must be at least 80000 kbps for 8K streaming")
        elif width >= 3840 or height >= 2160:
            if self.stream_bitrate < 12_000:
                raise ValueError("STREAM_BITRATE must be at least 12000 kbps for UHD streaming")

        if self.stream_fps > 120:
            raise ValueError("STREAM_FPS must not exceed 120 frames per second")
