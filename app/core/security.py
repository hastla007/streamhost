...
def generate_csrf_token(request: Request) -> str:
    ...
    digest = _hash_csrf(token)
    session = get_session_container(request)
    session[_CSRF_SESSION_KEY] = digest
...
def validate_csrf(request: Request, token: str | None) -> None:
    ...
    token_digest = _hash_csrf(token)

    if secrets.compare_digest(expected, token_digest):
        session.pop(_CSRF_PREVIOUS_KEY, None)
        return

    if previous and secrets.compare_digest(previous, token_digest):
        session.pop(_CSRF_PREVIOUS_KEY, None)
        return
